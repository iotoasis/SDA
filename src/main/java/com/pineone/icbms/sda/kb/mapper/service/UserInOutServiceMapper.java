...
public class UserInOutServiceMapper implements OneM2MMapper {
	...
	public void initResource() {
		this.user = model.createResource(this.baseuri + "/campus/" + this.dto.getUser_id());
		this.createDate = model.createTypedLiteral(StrUtils.makeXsdDateFromOnem2mDate(this.strDate),
				XSDDateType.XSDdateTime);
		this.location = model.createResource(this.baseuri + "/campus/" + this.dto.getZone());

		this.direction = model.createResource(this.baseuri + "/campus/" + this.dto.getDirection());
	}
	...
	public List<Statement> from() {
		...
		Statement typestmt = model.createStatement(this.userinout, RDF.type,
				model.createResource(baseuri + "/campus/GoInOut"));
		slist.add(typestmt);
		...
		Statement directionstmt = model.createStatement(this.userinout,
				model.createProperty(baseuri + "/campus/hasDirection"), this.direction);
