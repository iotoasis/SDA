...
// thread로 돌리는 경우
public class AvroRdbmsDeviceInfoSubscribe implements Serializable  {
	private static final long serialVersionUID = -3945383703195143703L;
	private final String TOPIC = Utils.KafkaTopics.COL_RDBMS.toString();
	private final Log log = LogFactory.getLog(AvroRdbmsDeviceInfoSubscribe.class);
	...
	//private final SchComm schComm = new SchComm();
	
	private final String user_id =this.getClass().getName();
	private final String group_id = this.getClass().getSimpleName();
	
	public static void main(String[] args) throws Exception{
		AvroRdbmsDeviceInfoSubscribe avroRdbmsDeviceInfoSubscribe = new AvroRdbmsDeviceInfoSubscribe();
		avroRdbmsDeviceInfoSubscribe.collect();
	}
				...
				public void run() {
					for(MessageAndMetadata<byte[], byte[]> messageAndMetadata : stream) {
						...
						try {
							 ...
							 if(colFrom.equals(Utils.COL_SS_DEVICEINFO_DATA)) {
								 processing_ok = true;
							 } 
