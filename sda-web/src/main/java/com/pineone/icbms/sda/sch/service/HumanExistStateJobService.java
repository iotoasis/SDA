 ...
 */
@Service
public class HumanExistStateJobService extends SchedulerJobComm implements Job {
	...
	public void runner(JobExecutionContext jec) throws Exception {
		...
		log.info("HumanExistStateJobService(id : "+jec.getJobDetail().getName()+") start.......................");
		...
		try {
			...
			for(int m = 0; m < argsResultList.size(); m++) {
				String conditon = argsResultList.get(m).get("cond");
				String location = argsResultList.get(m).get("loc");
				
				//delete->insert
				((SparqlFusekiQueryImpl)sparqlService.getImplementClass()).updateSparql(aggrList.get(0).getDeleteql(), aggrList.get(0).getInsertql(), new String[]{location, conditon}, Utils.QUERY_DEST.ALL.toString());
			...
			log.info("HumanExistStateJobService(id : "+jec.getJobDetail().getName()+") end.......................");			
		} catch (Exception e) {
