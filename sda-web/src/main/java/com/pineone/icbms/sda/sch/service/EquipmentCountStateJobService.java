...
public class EquipmentCountStateJobService extends SchedulerJobComm implements Job {
	...
	public void runner(JobExecutionContext jec) throws Exception {
		...
		log.info("EquipmentCountStateJobService(id : "+jec.getJobDetail().getName()+") start.......................");
...
//			List<Map<String, String>> aggrResultList;
			...
			for(int m = 0; m < argsResultList.size(); m++) {
				String lackCnt = argsResultList.get(m).get("lackCnt");
				String inventory  = argsResultList.get(m).get("inventory");
//				String equipDevice = argsResultList.get(m).get("equipDevice");
				
				//update
				//sparqlService.updateSparql(aggrList.get(0).getUpdateql(), new String[]{location, context_cond});
				
				//delete->insert
				((SparqlFusekiQueryImpl)sparqlService.getImplementClass()).updateSparql(aggrList.get(0).getDeleteql(), aggrList.get(0).getInsertql(), new String[]{inventory, lackCnt}, Utils.QUERY_DEST.ALL.toString());
			...
			log.info("EquipmentCountStateJobService(id : "+jec.getJobDetail().getName()+") end.......................");			
		} catch (Exception e) {
